...
- The blend weight `alpha` is tunable in the notebook
...
|---|---|
| `02_surprise_svd.ipynb` | Surprise SVD — matrix factorization baseline (requires `scikit-surprise`) |
| `03_lightfm_hybrid.ipynb` | LightFM WARP loss + content hybrid (requires `lightfm`) |
| `04_implicit_als.ipynb` | Implicit ALS — fully runnable with current environment |
| `05_recommendation_model.ipynb` | Full pipeline — EDA, outlier detection, feature engineering, model comparison, final recommendations |

Run `05_recommendation_model.ipynb` top-to-bottom to reproduce all results. Use `get_recommendations(user_id, top_n)` in the final section to generate recommendations for any user.
...
### Current limitations
- **Alabama only** — all listings are from a single state, so geographic diversity is limited to city-level differences
